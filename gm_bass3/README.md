...
| BASS3.SetProxy()      | ``string IP = ""``<br><br>``[number Port]``<br><br>``string user = ""``<br><br>``string password = ""`` | ``nil``                                                                 | Set the Proxy settings.<br><br>When the IP is set and not an empty string you have to specify the port.<br><br>Not giving the user name will create an anonymous connections.<br><br>When the proxy connection is not valid every online stream will yield error:<br>``BASS3.ENUM.ERROR_UNKNOWN`` |
