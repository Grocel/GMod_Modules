...
| IBASS3Channel:GetLevel()          | ``float TimeFrame = 0.02``<br><br>``bool RMS = false``<br><br>``bool AsDecibel = false`` | ``float Left``<br><br>``float Right`` | Returns the sound levels.<br><br>Returned values: 0 ... 1 OR -1000 db ... 0 db<br><br>The time frame value definites how much data to inspect to calculate the level.<br>Max: 1, Min: 0.001<br><br>If RMS is true, it will return the RMS instead of the peak levels.<br><br>The values are returned as decibel base 10 figures if AsDecibel is true. |
